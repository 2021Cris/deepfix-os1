...
// API Key para Vercel (Variable de entorno)
    ...
    <div className="flex h-screen bg-slate-950 text-slate-100 font-sans overflow-hidden">
      ...
      <aside className="w-72 border-r border-slate-800 bg-slate-900/40 p-8 flex flex-col">
        ...
        <div className="mt-auto space-y-4 pt-10 border-t border-slate-800">
          <p className="text-[9px] font-black text-slate-600 uppercase text-center tracking-widest">Cambiar Perfil</p>
      ...
      <main className="flex-1 overflow-y-auto p-16 custom-scrollbar">
        {view === 'dashboard' ? (
          <div className="space-y-12 animate-in fade-in duration-500 max-w-6xl mx-auto text-left">
            <header className="flex justify-between items-end">
              <div>
                <h2 className="text-4xl font-black text-white tracking-tighter uppercase leading-none">Resumen Operativo</h2>
                <p className="text-slate-500 text-sm font-bold uppercase tracking-widest mt-2">Plataforma deepfix-os1</p>
              </div>
              <div className="flex gap-4">
                 ...
                 {perms.canManage && <button onClick={() => setIsNewOrderModalOpen(true)} className="bg-blue-600 hover:bg-blue-500 px-6 py-3 rounded-2xl text-[10px] font-black uppercase shadow-xl shadow-blue-900/40 transition-all">Apertura OT</button>}
              </div>
            </header>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 animate-in slide-in-from-bottom-6 duration-700 max-w-7xl mx-auto pb-20 text-left">
                ...
                <div className="space-y-10 animate-in slide-in-from-right-8 duration-500">
                  <header className="flex justify-between items-start border-b border-slate-800 pb-10">
                    <div>
                      ...
                      <p className="text-slate-500 text-[10px] uppercase font-black mt-3">Iniciado: {selectedOrder.logs[0].date}</p>
                    </div>
                  ...
                  <div className="space-y-4">
                    <label className="text-[10px] font-black text-slate-600 uppercase tracking-[0.4em] flex items-center leading-none"><Paperclip size={14} className="mr-2" /> Evidencia Técnica</label>
                    <div className="grid grid-cols-1 gap-2">
                      ...
                      {perms.canExecute && selectedOrder.status !== 'Cerrada' && (
                        <label className="cursor-pointer border-2 border-dashed border-slate-800 p-8 rounded-3xl flex flex-col items-center hover:bg-blue-500/5 transition-all text-slate-500 hover:text-blue-400">
                          ...
                          <span className="text-[9px] font-black uppercase">Subir Documento / Foto</span>
                  ...
                  {selectedOrder.status === 'Asignada' && perms.canExecute && (
                    <div className="pt-10 border-t border-slate-800 space-y-8">
                       <label className="text-[10px] font-black text-blue-400 uppercase tracking-[0.4em] flex items-center leading-none"><Wrench size={18} className="mr-3"/> Análisis y Ejecución</label>
                       <form onSubmit={(e) => {
                          e.preventDefault();
                          const cost = parseFloat(e.target.cost.value) || 0;
                          const updates = { workDetail: e.target.work.value, cost };
                          if (cost > UMBRAL_APROBACION && !selectedOrder.isAuthorized) {
                            handleUpdateOT(selectedOrder.id, { ...updates, status: 'Pendiente Autorización' }, 'Excedente de costo: Solicitando aprobación');
                          } else {
                            handleUpdateOT(selectedOrder.id, { ...updates, status: 'Cerrada' }, 'Mantenimiento finalizado con éxito');
                          }
                       }} className="space-y-6">
                          <textarea name="work" required rows="4" className="w-full bg-slate-800 border border-slate-700 rounded-3xl p-8 text-sm outline-none font-medium leading-relaxed text-white focus:ring-2 focus:ring-blue-500 shadow-inner" placeholder="Escriba aquí los detalles técnicos de la reparación..."></textarea>
                          <div className="grid grid-cols-2 gap-6">
                            <div className="space-y-2">
                                <label className="text-[9px] font-black text-slate-600 uppercase ml-4 tracking-widest">Costo de Trabajo ($)</label>
                  ...
                  {selectedOrder.status === 'Pendiente Autorización' && perms.isAutorizador && (
                    <div className="pt-10 border-t border-slate-800 text-center space-y-6">
                       ...
                       <p className="text-xs text-slate-400 italic">Monto de ${selectedOrder.cost.toLocaleString()} requiere su validación oficial.</p>
                       <button onClick={() => handleUpdateOT(selectedOrder.id, { status: 'Asignada', isAuthorized: true }, 'Costo autorizado por jefe de área')} className="w-full bg-purple-600 hover:bg-purple-500 py-6 rounded-[2rem] font-black text-xs uppercase text-white shadow-xl flex items-center justify-center transition-all">
                  ...
                  <div className="pt-12 border-t border-slate-800/50">
                    <h4 className="text-[10px] font-black text-slate-600 uppercase tracking-[0.4em] mb-10 flex items-center leading-none italic font-bold tracking-widest"><History size={16} className="mr-3" /> HISTORIAL DE MANTENCIÓN</h4>
                      ...
                      {(selectedOrder.logs || []).map((log, idx) => (
                        <div key={idx} className="flex gap-8 relative text-left">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest italic opacity-80 leading-none">{log.user} — {log.date}</p>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <form onSubmit={handleCreateOrder} className="space-y-10">
              <div className="grid grid-cols-2 gap-8 text-left">
                 <div className="space-y-2">
                    ...
                    <select name="vehicleId" className="w-full bg-slate-800 border border-slate-700 rounded-3xl px-8 py-5 text-sm font-bold text-white outline-none focus:ring-2 focus:ring-blue-500 appearance-none shadow-inner">
                  ...
                  <div className="space-y-2">
                    ...
                    <select name="category" className="w-full bg-slate-800 border border-slate-700 rounded-3xl px-8 py-5 text-sm font-bold text-white outline-none focus:ring-2 focus:ring-blue-500 appearance-none shadow-inner">
